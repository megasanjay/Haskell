...
proper (Union' rs) = all proper rs  --and [ proper r | r <- rs]
proper (Cat' rs) = any proper rs --or [proper r | r <- rs]
proper (Star' r) = False
...
--lIJ = [[simp (coef i j) | j <- qs] | i <- qs]
  --coef i j = undefined -- union prime outermost function use union prime on list comprehension

-- Construct constants
lI' = undefined -- every element in the list is a 1 or 0
