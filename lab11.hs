-- Lab 11: Derivative-based conversion from RE' to FSM (Brzozowski Construction)
...
byp Zero        = True
byp (Letter' c) = False
byp (Union' r)  =  or [ byp x | x <- r]--byp r || byp (Union' rs)
...
deriv a (Union' r) = Union' [deriv a x | x <- r]
--deriv a (Union' r) = Union' ([deriv a (head r)] ++ deriv a Union' (head ))
--deriv a (Union' r) = Union' ([deriv a (head r)] ++ (deriv a (Union' (tail r))))
--deriv a (Union' (r:rs)) = Union' [(deriv a r),rs]
---deriv  a (Cat; ) = Union' (Cat' [deriv a r] ++ rs) ++ [deriv a x | x<- rs]
--deriv a (Cat' r:rs) = if byp r then simp $ Union' (Cat' [deriv a r] ++ ())
deriv a (Cat' (r:rs)) = if byp r then Union' (Cat' [deriv a r] ++ rs) : [deriv a x | x<- rs] else simp $ Cat' ([deriv a r] ++ rs)
--deriv a (Cat' r) = if byp (head r) then Union' ((Cat' r) ++ [deriv a Cat'(tail r)])  else Cat' ([deriv a (head r)] ++ tail r)
--deriv a (Cat' (r:rs)) = if byp r then Union' (Cat' [One, deriv a rs]) else Cat' ([deriv a r] ++ rs)
--deriv a (Cat' [r1,r2]) = if byp r1 then Union' [(Cat' [r1,r2]),(deriv a r2)] else Cat' [(deriv a r1),r2]
